...
@pytest.fixture()
def row_for_dx_dicom_testing(db_session) -> Session:
    """
    #similar to row_for_dx_dicom_testing chekcing for Modality="DX"
    Insert a test row for the fake DICOM dataset generated by
    pytest_pixl.dicom.generate_dicom_dataset.
    """
    extract = Extract(slug=TEST_PROJECT_SLUG)
    ds = pytest_pixl.dicom.generate_dicom_dataset(Modality="DX")
...
def row_for_mr_dicom_testing(db_session, yaml_file) -> Session:
    """
    Insert a test row for the fake DICOM dataset generated by
    pytest_pixl.dicom.generate_dicom_dataset.
    """
    ...
    if modality == "MR":
        ...
        ds = pytest_pixl.dicom.generate_dicom_dataset(Modality="MR")
        study_info = get_study_info(ds)
...
@pytest.fixture()
def vanilla_dicom_image_DX(row_for_dx_dicom_testing) -> Dataset:
    """
...
@pytest.fixture()
def vanilla_dicom_image_MR(row_for_mr_dicom_testing) -> Dataset:
    """
    A DICOM image with diffusion data to test the anonymisation process.
    Private tags were added to match the tag operations defined in the project config, so we can
    test whether the anonymisation process works as expected when defining overrides.
