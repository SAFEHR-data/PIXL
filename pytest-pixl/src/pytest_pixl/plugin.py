...
#
# However, some tests initiate FTP connections from pytest (running on host) rather than containers.
# These require the server to be bound to localhost. So the test itself needs to tell us where to
# bind (using a fixture called ftp_host_address)
#
...
def ftps_server(tmp_path_factory, ftp_host_address: FtpHostAddress) -> Generator[PixlFTPServer, None, None]:
    """
    ...
    ftps_server = PixlFTPServer(home_root=tmp_home_dir_root,
                                host_address=ftp_host_address.to_host_ip_address())
